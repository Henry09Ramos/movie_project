...
import {ICreateGenero, IGetGeneros} from '../../types/genero.types'
...
setShowAlert(true)
//en el siguiente codigo podemos agregar un temporizador para ocultar la alerta  despues de un tiempo 
setTimeout(()=>{
    setShowAlert(false)

},3000 //oculta la alerta despues de haber pasado unos 3000 milisegundos(3segundos)
)
...
const onSubmit = (data: ICreateGenero) => {
    ...
    get_genero(data)
      .then((data) => {
        console.log('Genero has been successfully created')
    ...
    {showModal &&(
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
            <div className="bg-white rounded.lg shadow-lg p-6">
                <h3 className="text-lg font-medium mb-4 text-center">Agregar Genero</h3>
                <form  onSubmit={handleSubmit(onSubmit)} >
                    <div className="mn-4" >
                        <label htmlFor='genero' className="block text-gray-700 text-sm font-medium">Genero: </label>
                        <input {...register("type")} className="w-full h-10 p-4 border rounded-xl" placeholder="Inserte el genero"/>
                    </div>
